...
| # | 文書 | 内容 |
|---|---|---|
| 01 | [GDR 仕様](docs/ja/01_GDR/01_GENERAL_DECISION_RECORD.md) | GDR の定義・目的・書式・scope と PREFIX |
| 02 | [AI-Driven GDR ビルドアップ](docs/ja/01_GDR/02_AI_DRIVEN_GDR_BUILDUP.md) | ビルドアップの定義・サイクル・原則・導入方法 |
| G-00 | [Getting Started](docs/ja/01_GDR/guide/00_GETTING_STARTED.md) | GDR を最短で始めるためのガイド |
| G-01 | [scope ガイド](docs/ja/01_GDR/guide/01_SCOPE_GUIDE.md) | scope ガイドライン（共通 + 業種別） |
...
3. [ビルドアップ記録](docs/ja/01_GDR/templates/T2_BUILDUP_RECORD.md)にサイクルの流れを追記していく

詳細は [AI-Driven GDR ビルドアップ § 導入方法](docs/ja/01_GDR/02_AI_DRIVEN_GDR_BUILDUP.md#9-導入方法) を参照してください。

## ディレクトリ構成

```
docs/                                      # Docsify サイト（GitHub Pages 用）
├── index.html                             # Docsify エントリポイント（ルート）
├── ja/                                    # 日本語版
│   ├── index.html                         # Docsify エントリポイント（日本語）
