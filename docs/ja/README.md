# GDR ドキュメント（日本語）

このサイトのトップページは [GDR 仕様](/01_GDR/01_GENERAL_DECISION_RECORD.md) です。

## 主要文書

- 仕様
  - [01. GDR とは](/01_GDR/01_GENERAL_DECISION_RECORD.md)
  - [02. AI-Driven GDR ビルドアップ](/01_GDR/02_AI_DRIVEN_GDR_BUILDUP.md)
- ガイドライン
  - [00. Getting Started](/01_GDR/guide/00_GETTING_STARTED.md)
  - [01. scope ガイドライン](/01_GDR/guide/01_SCOPE_GUIDE.md)
  - [02. PREFIX ガイドライン](/01_GDR/guide/02_PREFIX_GUIDE.md)
  - [03. ローカライズ初回プロンプト](/01_GDR/guide/03_FIRST_PROMPT_GUIDE.md)
  - [04. プロンプトガイドライン](/01_GDR/guide/04_PROMPT_GUIDE.md)
- テンプレート
  - [T1. Context Definitions](/01_GDR/templates/T1_CONTEXT_DEFINITIONS.md)
  - [T2. ビルドアップ記録](/01_GDR/templates/T2_BUILDUP_RECORD.md)

> Docsify サイトとして配信されています。GitHub 上で直接 markdown を読む場合、絶対パスのリンク（`/01_GDR/...`）はリンク切れに見えますが、Docsify 経由では正しく解決されます。
